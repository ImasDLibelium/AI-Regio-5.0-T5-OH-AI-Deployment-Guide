...
Example of using cURL to upload the model:
	...
	curl -X POST http://<ESP32_IP>/uploadModel -F "model=@path/to/your/model.tflite"  or use applications such as Thunder(VSCode) or Postman.
...
Example of using cURL to send the configuration:
	...
	curl -X POST http://<ESP32_IP>/configure -H "Content-Type: application/json" -d @config.json
